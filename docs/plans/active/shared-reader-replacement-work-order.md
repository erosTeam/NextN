...
- **Identified implementation and acceptance gaps (Package 5 OPEN):**
  ...
  4. *NextE anchor/seek reset to page 1 in two trials (NEW, deterministic, root cause not isolated):* on 197 at pin `f450aad`, (a) tracked `ReaderSpreadLayoutTrial` fails at `ReaderSpreadLayout.test.ets:233` — after a slider seek to the last page, `rkit-chrome-page` reads 1, expected 46 (earlier joined/split/rotation assertions at lines 149-223 passed); (b) `ReaderPagingAxisTrial` (untracked; never passed) fails at `ReaderPagingAxis.test.ets:115` — after vertical paging (page 2, `vertical-p2.png`) and a portrait return (page 2, asserted line 110), switching back to single layout then swiping leaves page 1, expected 3. Evidence (`.hvigor/outputs/reader-display-modes/.../{05-vertical,06-vertical-rerun,07-spread-layout}/`). Not caused by `f450aad` (that delta only touches failure state; neither trial involves a failure). Investigation so far: the tracked `ReaderCropSpreadTrial`/`ReaderCropContinuousTrial` pass the analogous menu-switch-and-keep-page assertion, and re-running `ReaderSliderLifecycleTrial` hit *trial* brittleness first (an async runtime-menu row read without a wait at `:40`, then an unguarded `driver.dumpLayout`/`captureSeek` and a `rkit-chrome-page`-not-yet-mounted read at `:17`) rather than an anchor reset — I reverted those speculative trial edits instead of shipping them. So the failing steps are a slider seek-to-end and a post-switch swipe. Note (spread trial): the slider seek at `:230-231` happens while the spread is still **zoomed** (double-tapped at `:136`), so the seek-to-last-page path under zoom is the specific suspect. Whether it exposes a shared seek/zoom/policy-change defect or only trial brittleness is not yet isolated. Recorded OPEN rather than fixed or ignored.
