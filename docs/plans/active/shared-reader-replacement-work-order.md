...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Comic Visual Translation** | NextE | `ReaderPage` + `ComicTranslationRuntimeService` orchestrates OCR/Inpainting/TextTranslator | `NextEReaderTranslationProvider` + `variant: 'translated'` generation swaps; `translate-page` / `translate-auto` host actions | `test_reader_translation_provider_runtime.mjs`; `ReaderHostTranslationActionsTrial` (`d0c24aed`) on 197 passing (14.7 s, `.hvigor/outputs/nexte-translation-actions/.../04-host-translation-retried/`) with 4 captures reviewed (`actions.png`, `dispatched.png`, `followup.png`, `retried.png`) | **PARTIALLY ADVANCED / OPEN:** Service request arrival physically confirmed by live log `[comic-translation] source_identity_resolved | reader=1280x1807 file=1280x1807`. Menu open, initial translate click, post-dispatch viewport retention, menu followup re-acquisition, and actual retry click execution are verified on real content 4175844. End-to-end cloud inference and rendered text overlay display remain OPEN depending on live backend service configuration. |
