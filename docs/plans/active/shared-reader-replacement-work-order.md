...
- **Identified implementation and acceptance gaps (Package 5 OPEN):**
  ...
  4. *NextE mixed layout/rotation trial fails deterministically (NEW, root cause not isolated):* `ReaderPagingAxisTrial` (untracked in this work order; never passed) fails on 197 at `ReaderPagingAxis.test.ets:115` — after vertical paged reading (page 2/46 shown in `vertical-p2.png`), a landscape→portrait rotation (page 2 preserved, asserted at line 110), then a switch back to single layout via the runtime menu, a forward horizontal swipe leaves the page at 1 instead of 3. Reproduced twice (`.hvigor/outputs/reader-display-modes/.../{05-vertical,06-vertical-rerun}/`). Not yet attributed: it is either a shared read-anchor-preservation defect on the layout switch or trial brittleness (the runtime-menu tap in a changed layout). Recorded OPEN rather than fixed or ignored.
