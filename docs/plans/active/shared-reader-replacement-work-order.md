...
- NextN **shared production-overlay system Back needs two presses (legacy needs one) — open counterexample** (2026-09-18, 197, candidate `9e3e5793` reader-kit `f450aad`): entering the ordinary production shared route (`nextn_reader_backend=shared` + `nextn_gallery_id=678049` → Detail Read action → `openReaderOverlay` shared destination) and injecting the system Back key (`uitest uiInput keyEvent 2`) does **not** close the shared reader on the first press; a second Back closes it. Measured on device: after Back #1 the reading surface is still mounted (19 `rkit-*` nodes, incl. `rkit-reading-surface`/`rkit-chrome-top`/`rkit-chrome-page`), even after a 6 s settle, and a warm-up VOLUME key then a single Back also leaves 19 nodes; after Back #2 the route closes (0 `rkit-*`). The **legacy control on the same route** closes on a single Back (`legacy-reader-surface` 1 → 0 after one press). Source: the shared overlay's Back is delegated through `Index.onBackPress → readerBackRequest` (registered by `NextNReaderLabPage.onRegisterBackRequest` → `closeTrial`), so the handler exists; the first press is not reaching/acting on it in this run. This is recorded as an **open counterexample** — a shared-route Back difference against the legacy control, **not** a confirmed product defect and **not** a pass — with no speculative rewrite shipped. Artifacts: `.hvigor/outputs/nextn-backkey/run4/{reader,b1,b2}.json` (two-press), `.hvigor/outputs/nextn-backkey/run6-slow/b1.json` (single Back + 6 s, still 19), `.hvigor/outputs/nextn-backkey/run7-focus/after-single.json` (warm-up + single Back, still 19), `.hvigor/outputs/nextn-backkey/run5-legacy/{reader,b1}.json` (legacy one-press control). Koma's lab shared route closes on a single Back (recorded above), so this is a **NextN overlay-route** question to isolate further, not a cross-host shared-layer conclusion.
