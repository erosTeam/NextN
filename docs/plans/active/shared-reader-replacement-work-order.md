...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Comic Visual Translation** | NextE | `ReaderPage` + `ComicTranslationRuntimeService` orchestrates OCR/Inpainting/TextTranslator | `NextEReaderTranslationProvider` + `variant: 'translated'` generation swaps; `translate-page` / `translate-auto` host actions | `test_reader_translation_provider_runtime.mjs`; extended `ReaderHostTranslationActionsTrial` (`939777c4`) on 197 passing with menu, dispatch, and followup captures reviewed (`.hvigor/outputs/nexte-translation-actions/.../03-host-translation-followup/`) | **PARTIALLY ADVANCED / OPEN:** Host action exposure, live dispatch invocation, viewport stability, and followup retry menu exposure are physically verified on real content 4175844. End-to-end external cloud inference and rendered text overlay display remain dependent on live backend service configuration. |
...
| **Chapter Orchestration** | Koma | Legacy `ReaderPage` monitors chapter sequence and loads next chapter on edge | `KomaReaderLabPage` handles `ReaderChapterNavigation`, invokes `switchChapter`, and mounts `ReaderCenterAction` | `test_shared_reader_catalog_host.cjs`; 197 real-content ordinary entry and chapter-switch pass | **CLOSED (Parity verified):** Explicit previous/next chapter preparation, cancellation, rapid A-B-C selection, terminal semantics, and read state persistence pass. Smooth weak-network preloading remains a progressive enhancement. |
| **Native Thumbnail Transition** | NextN | `ReaderThumbnailTransitionCoordinator` maps thumbnail bounds to reader viewport | Same coordinator hooked via `ReaderCloseContext` and `prepareReaderDestinationClose` in `Index.ets` | `test_gallery_reader_transition_contract.mjs`; ordinary entry and rotation layout captures | **OPEN (Transition Dynamic Gap):** Static layout captures prove endpoint geometry and coordinate binding; transition animation smoothness and coordinator flight under rapid interaction remain to be verified dynamically. |
| **Common Controls & Chrome** | All three hosts | Separate custom chrome implementations | Unified `ReaderSurface` (NextN:808, NextE:750, Koma:595) with shared top/bottom toolbars, slider, and page labels | Shared surface mounted on all three hosts | **OPEN (Cross-host Same-state Parity Gap):** Shared component reuse is verified, but side-by-side same-state visual evidence across all three apps remains open to ensure intentional host differences do not conceal accidental divergence. |
