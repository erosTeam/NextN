...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Comic Visual Translation** | NextE | `ReaderPage` + `ComicTranslationRuntimeService` orchestrates OCR/Inpainting/TextTranslator | `NextEReaderTranslationProvider` + `variant: 'translated'` generation swaps; `translate-page` / `translate-auto` host actions | `test_reader_translation_provider_runtime.mjs`; `126e067f` (fixes initial `currentSourceIndex` sync & `rkit-host-status` formatting); `ReaderHostTranslationActionsTrial` (`05-host-translation-status-mounted`) on 197 with 4 full captures reviewed | **PARTIALLY ADVANCED / OPEN (UI Status Mounted & Service Dispatched):** Service request arrival physically confirmed by live log `[comic-translation] source_identity_resolved`. `126e067f` closes the UI bug where `rkit-host-status` failed to render when translation fails without LLM configured. Menu open, translate click, host status mounting, and retry click execution verified on real content 4175844. Cloud inference with live LLM credentials remains an optional backend configuration outside core reader UI boundary. |
