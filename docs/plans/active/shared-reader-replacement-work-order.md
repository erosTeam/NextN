...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Comic Visual Translation** | NextE | `ReaderPage` + `ComicTranslationRuntimeService` orchestrates OCR/Inpainting/TextTranslator | `NextEReaderTranslationProvider` + `variant: 'translated'` generation swaps; `translate-page` / `translate-auto` host actions | `test_reader_translation_provider_runtime.mjs`; `ReaderHostTranslationActionsTrial` (`939777c4`) on 197 passing with menu, post-click viewport, and conditional followup captures reviewed (`.hvigor/outputs/nexte-translation-actions/.../03-host-translation-followup/`) | **OPEN (Acceptance Gap):** Menu click preserves reader surface and allows conditional menu reinspection. True service request arrival, actual translation result rendering, original/translated toggling roundtrip, and actual retry click remain unverified. E2E live translation stays OPEN; next step must obtain actual service return/page outcome or document exact invocation failure without speculating on missing API/weights. |
