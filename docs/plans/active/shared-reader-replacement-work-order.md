...
- NextE **lab-route `ReaderOriginal*` trials — uncleared counterexamples** (2026-09-18, 197, candidate `516c60ed` reader-kit `f450aad`): `ReaderOriginalSingleTrial` failed at its first control lookup `ReaderImageInformation.test.ets:675` (`expect(action !== null && await action.isEnabled()).assertTrue()` false) and `ReaderOriginalSpreadTrial` failed at `:612` (`Cannot read property click of null` on `rkit-toggle-spread`) — both short (3.2 s / 4.9 s), before any original-variant assertion. The failure-point layout dump contained **no** `rkit-*` nodes, so the shared chrome was not mounted when the lookup ran. The **production** original path is separately device-verified at the same pin (`ReaderProductionOriginalMenuTrial` passes 1/1 and opens the 原图 action menu over the real RTL spread, recorded above), so this is recorded as an **open lab-route counterexample** (trial/lab configuration vs. fixture/route timing), **not** a product-defect claim and **not** a pass; no speculative trial rewrite is shipped. A USB-transport retry on the same device failed **again but at different points** (single: the image-information dialog title was not found at `ReaderImageInformation.test.ets:69`; spread: `rkit-toggle-spread` null at `:612`), confirming these lab-route trials are **flaky against their own chrome/timing preconditions**, not a stable product gap. Failed runs retained: `.hvigor/outputs/nexte-orig-single-f450aad/run1/run-metadata.json`, `.hvigor/outputs/nexte-orig-spread-f450aad/run1/run-metadata.json`, and the USB retries `.hvigor/outputs/nexte-orig-{single,spread}-f450aad/run-usb-r2/run-metadata.json`.
...
- **NH partial-thumbnail geometry preserved — current-pin device probe (2026-09-18, ALN-AL80 / 197 over USB)**: opening the real long-strip gallery 678049 in the NextN Detail page and dumping the thumbnail strip shows the per-page thumbnail tiles at their **own decoded aspects**, not a fixed original ratio — e.g. `reader-thumb-gallery-detail-1-page-{0,1,2}` measure `317 x 488` (ratio 0.650) while a narrow page `-page-3` measures `114 x 488` (ratio 0.234). This is the objective's required NH behaviour (partial thumbnails keep their decoded thumbnail aspect with bounded cover/contain) observed directly at the pin, distinct from NextE's EH sprite-crop contract. Artifact: `.hvigor/outputs/nextn-detailthumb-probe/run1/detail.json`.
- NextE **information-supplement and spread-shift-last — open counterexamples** (2026-09-18, device ALN-AL80 / 197 over USB, candidate `516c60ed` reader-kit `f450aad`): `ReaderInformationSupplementTrial` failed at `ReaderImageInformation.test.ets:232` on `expect(message.indexOf('图像增强：关闭') >= 0)` — the shared image-information dialog did not contain the expected 「图像增强：关闭」 line in that run (an information-panel content/extraction question, not a reading-path defect); `ReaderSpreadShiftLastTrial` failed at `:471` with `rkit-shift-spread` null after the trial's own RTL/enable sequence. A USB retry reproduced both **deterministically at the same assertions** (supplement again at `:232` on the `图像增强` line; shift-last again at `:468` with `rkit-shift-spread` present where the trial expects it null). Source inspection isolates a **trial coupling, not a shared-reader defect**: `ReaderImageInformation.test.ets` hardcodes the **zh_CN** dialog title `图片信息 · 第 N 页` (`:62`), while `ReaderSurface.ets:466` builds that title from the **locale-dependent** resource `rkit_info_title` (base/zh_CN `图片信息 · 第 N 页`, en_US `Image information · Page N`, ja_JP `画像情報 · N ページ`), so any non-zh_CN app locale makes the title lookup fail; and the supplement lines come from the host `mediaActions.informationSupplement` (`NextEReaderLabPage.ets:118`). The main `ReaderImageInformationTrial` (same helper) also failed at its title lookup (`:69`) on a fresh run at this candidate even though it had **passed 1/1 at candidate `b7d6d03f`** with pixel evidence showing the exact Chinese dialog (「图片信息 · 第 2 页」 with 格式/大小/当前/原图/图像增强/图源) — and **no product source changed** since `b7d6d03f` (only two test-only commits). The reader-route timing (`informationReady` waits only on an enabled `rkit-more`; the failure-point capture resolved to the host Home, i.e. the reader route had already cycled) is the more likely cause of the supplement/`:232` miss than the locale (the supplement label is itself zh_CN-consistent). The `图像增强：关闭` content question is now **closed by direct device evidence** (see the supplement-lines record below): a manual drive of the same shared dialog at this pin renders exactly `图像增强：关闭` and `图源：…`. So the shared dialog content is correct and only the **trial/method** (hardcoded locale title + reader-route timing) is brittle. The `ReaderSpreadShiftLastTrial` root cause is also isolated: it asserts `rkit-shift-spread` is **absent** at launch (`ReaderImageInformation.test.ets:468`), but at this pin the NextE reader opens in the **device-persisted double-page spread** — a direct layout dump shows both `rkit-part-0-whole` + `rkit-part-1-whole` mounted and `rkit-shift-spread` present on entry — so the trial's "opens single-page" assumption is violated by the host's persisted layout, the same trial-geometry brittleness already documented for the NextE spread/paging-axis trials (which were fixed by pinning a Debug entry-layout Want this harness does not set). Both remain **open counterexamples** (the sibling `ReaderSpreadShiftTrial`, which differs only in the seeded page, has a prior pass), not passes and not defect claims; no speculative rewrite is shipped, and the failure-point catches are retained. Failed runs: `.hvigor/outputs/nexte-{supplement,shiftlast,infodialog}-f450aad/run-usb*/run-metadata.json`, retries `.hvigor/outputs/nexte-{supplement,shiftlast}-f450aad/run-usb-r2/run-metadata.json`.
- NextE **share page-cancel + original-variant share — lab counterexamples** (2026-09-18, 197, candidate `516c60ed` reader-kit `f450aad`): two share trials failed on lab-route preconditions, not product defects. `ReaderSharePageCancelTrial` failed at `ReaderImageShare.test.ets:98` (`expect sharePage === 3`, got 2) — its hardcoded 0.8-width tap did not advance the page on a device that persists **RTL** (that tap moves backward under RTL), the same trial-geometry brittleness already recorded for the NextE spread/paging-axis trials; those were fixed by pinning `readerLabEntryDirection=ltr`, which this share harness does not do. `ReaderOriginalVariantShareTrial` failed at `:152` on the `当前：原图` information assertion, i.e. the same lab-route original-selection limitation as the `ReaderOriginal*` counterexample above (the lab has no original-selection provider). Neither reached a shared-reader share defect, and no product change or speculative trial rewrite is shipped. Failed runs retained: `.hvigor/outputs/nexte-share-pagecancel-f450aad/run1/run-metadata.json`, `.hvigor/outputs/nexte-share-origvars-f450aad/run1/run-metadata.json`.
- NextE **read-only tap-zone / page-number trials — blocked by their own site precondition** (2026-09-18, 197, candidate `516c60ed` reader-kit `f450aad`): `ReaderTapZonesReadOnlyTrial` and `ReaderPageNumberReadOnlyTrial` both failed at their **first** assertion, `expect(connectSiteMode().isEx).assertFalse()` (source `ReaderTapZones.test.ets:51` / `ReaderPageNumber.test.ets:166`), after only ~1.2 s — before any shared-reader surface was opened. The device 197 persists its gallery site as **EX** (that is the real user site), while these two read-only trials hardwire the EH site in their launch Want (`readerLabSite: eh`) and assert the current site is EH. This is **trial brittleness against the device's real site setting, not a shared-reader defect**: neither trial reached the reader, and the sibling `ReaderPageNumberToggleTrial` — which does not gate on the site — already passes 1/1 at the pin and is the page-number pin evidence (see above). The unused `reader_tap_zone_layout` / `reader_tap_zone` plumbing these trials intend to check is separately covered: every legacy tap-zone action maps to the shared `ReaderTapPolicy` resolver (capability-parity audit above), and NextN's equivalent tap-zone trial passes 1/1 at the pin (its record above). No product change is made, and the user's persisted EX site is not altered. Failed runs retained as counterexamples: `.hvigor/outputs/nexte-tapzones-f450aad/run1/run-metadata.json`, `.hvigor/outputs/nexte-pagenumber-f450aad/run1/run-metadata.json`.
...
- NextN **image-information page-cancel recovery — open counterexample** (2026-09-18, 197, candidate `2ce25217` reader-kit `f450aad`): the `page` mode of the same harness (`ReaderInformationPageCancelTrial`, `readerLabInformationProbe=delay-once`) failed at `ReaderImageInformation.test.ets:38` inside `informationReady` — after the pending-information page turn, the shared `rkit-more` button never re-enabled within the 20 s window (`expect(false).assertTrue()`), having consumed ~36 s. The `failure` mode passed at the same pin and the `background` mode has a prior pinned record, so this is recorded as an **open counterexample** (delay-probe timing vs. a real re-enable gap) — **not** a pass and **not** a product-defect claim, with no speculative rewrite shipped. Failed run retained: `.hvigor/outputs/nextn-info-page-cancel-f450aad/run1/run-metadata.json`.
