...
- **Current-code ↔ evidence integrity check (2026-09-18)**: after this session's work the recorded device acceptance still bounds the current sources. reader-kit is unchanged at the pin `f450aad0f…` (identical gitlink in all three hosts); NextE's only commits since its pinned acceptance `516c60ed` are test/doc (`git log 516c60ed..HEAD -- feature entry/src/main shared` empty), and Koma's since `6553e2df` are none. NextN's single product change this session is the shared-Back guard (`23875573`→`33db6d5b`, `Index.handleGalleryDetailBackPressed` only); it is additively guarded and re-verified on device at both form factors plus the ordinary-entry matrix, Gallery-detail Back trial, in-app rehearsal, release fail-closed and thumbnail-entry trials on the fixed build, with all four contract suites green. So no device acceptance is stale against the current code, and the only re-open trigger is a future non-test edit to a reader open/admission, host Back, or shared-surface path.
