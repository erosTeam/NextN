...
- NextE **volume-key page turning** could not be device-verified in this run (2026-09-17; 197 was offline for a system update so testing switched to authorized 237): the tracked `ReaderVolumeKeysTrial` fails at its first `volumePage` read (`rkit-chrome-page did not appear`), and retrieving the trial's own `missing-page-label` capture shows the test-harness context sitting on the **NextE gallery root**, not the reader — i.e. the reader never mounted for the trial on 237 even though the same lab entry launched via `aa start` does mount it (`rkit-reading-surface` + `rkit-chrome-page 1 / 46` confirmed by `uitest dumpLayout` at 2 s/5 s/10 s, artifacts `.hvigor/outputs/nexte-237-recon/{lab,time2}/`). Classified as a test-harness/environment limitation on 237, not a product defect and not an accepted NextE volume-key pass. Speculative trial-side edits (LTR pin + readiness wait) were reverted; no NextE test or product change was kept. NextE volume-key runtime stays OPEN pending a 197 run (where the same trial's siblings mount the shared reader). Artifact: `.hvigor/outputs/nexte-237-volumekeys/recv2/missing.png`.
