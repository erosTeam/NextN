...
- NextN **continuous zoom/pan/reflow gestures** verified at the pinned revision (2026-09-18, 197 `ALN-AL80` 1260x2720, candidate `cea6bdff` reader-kit `f450aad`, `install -r` only): the tracked `ReaderLabGestures` continuous classes had **no record** in this work order (the file's eight classes cover paged pinch/pan/reset and the four continuous variants). Two are now device-verified on real long-strip gallery 678049 through the Debug lab entry: `ReaderLabContinuousReflow` (continuous mode, two-pointer pinch to zoom, a horizontal pan that must not advance the pager, rotate to landscape and back, pan again, pinch-reset and re-scroll; asserts the page stays `1 / ` and captures five checkpoints `zoomed-portrait/landscape/returned-portrait/post-rotation-pan/reset-scroll`) passes **1/1**; `ReaderLabContinuousPinchPan` (continuous mode pinch then a post-gesture horizontal pan that must pan the zoomed item rather than turn the page) passes **1/1**. Both report `native_injection_completed=<class>` and a final `rkit-page-position` of `1 / 14` in continuous layout (`rkit-layout=连续`, `rkit-continuous-list` mounted at `[39,863][1221,2171]`), so a continuous-reader pinch/pan does not silently change the reading position. No product source changed (no-record trials; nothing needed fixing). Honest scope: the lab surface's control column compresses the continuous body on the landscape checkpoint, so the landscape frame is a layout/mode check (continuous stays active, page identity `1 / 14`), not a body-geometry assertion; the four portrait checkpoints carry the zoom/pan/reflow pixels. Artifacts: `.hvigor/outputs/nextn-contreflow2-f450aad/run-197/` (five reflow checkpoints + native-pinch), `.hvigor/outputs/nextn-contpinchpan2-f450aad/run-197/`.
