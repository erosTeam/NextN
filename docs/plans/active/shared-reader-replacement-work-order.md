...
- NextE **volume-key page turning** is not verified (2026-09-17, re-run on both devices after 197 returned from its system update): the tracked `ReaderVolumeKeysTrial` (bulk-added in `3280ba90`, no prior pass record) fails on **both** 197 and authorized 237 at its first `volumePage` read with `rkit-chrome-page did not appear`, even after pinning the Debug-only `readerLabEntryDirection=ltr` (the sibling trials' fix) and adding a bounded ~10 s readiness wait — both edits were reverted as they did not produce a pass. The failure is at the **test-harness** boundary, not the product: the same NextE lab entry launched via `aa start` mounts the shared reader fine on both devices (`rkit-reading-surface` + `rkit-chrome-page 1 / 46` seen by `uitest dumpLayout`), and 237's `missing-page-label` capture showed the harness context on the gallery root rather than the reader. So NextE volume-key runtime stays **OPEN** pending a dedicated harness fix (out of scope for parity evidence) or a NextE test-side rewrite; it is **not** a product defect and **not** an accepted pass. Artifacts: `.hvigor/outputs/nexte-volumekeys/{run-197,run-197b,run2,recv2}/`, `.hvigor/outputs/nexte-237-volumekeys/recv2/missing.png`. (NextN's twin trial *is* verified at the pin — see the NextN volume-key record.)
