...
- **Identified implementation and acceptance gaps (Package 5 OPEN):**
  ...
  3. *Shared decode/render failure carries no host copy:* NextN now owns a classifier (`NextNReaderFailure` + `ReaderAssetFailureClassifier` on `NextNReaderLabAdapter`, session wired) and the acquisition path is device-verified on 197 (`6f99aa28`: `rkit-failure-page-1`/`rkit-retry-page-1` render 「图片不可用 / 请检查网络后重试」, `.hvigor/outputs/nextn-failure-copy/.../06-failure-load-copy/`). Still OPEN: `reader-core`'s `ReaderSession.reportPresentation(false)`/`reportRenderFailure` set `state.failure = null`, so a *decode/render* failure (not an acquisition failure) renders reader-kit's generic `rkit_image_failed`/`rkit_image_retry_hint` even though legacy NextN shows its own copy for that event. Fixing this needs a `reader-core` change plus a coordinated reader-kit pin bump across all three hosts, so it is recorded OPEN rather than patched unilaterally. The `reader-kit/tests/reader-failure-presentation.test.cjs` currently pins only the load-path classification port.
