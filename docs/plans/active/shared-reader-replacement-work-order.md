...
- NextN **crop (裁边)** at the pinned revision (2026-09-17, 197, candidate `0051eeb4` reader-kit `f450aad`): the tracked `ReaderCropSpreadTrial` had no pinned-revision record; it now passes **1/1**. On real long-strip gallery 678049, the trial discovers a croppable page (the log shows `ReaderCrop discovery page=5 detected=true`), confirms the same page with crop off then on renders the shared `rkit-cropped-image-5-whole` geometry (`ReaderCrop sample=5 detected=true`), advances the RTL spread by a volume key and returns, and asserts the source page is unchanged and the reader closes. Artifact: `.hvigor/outputs/nextn-spread-crop-f450aad/run1/run-metadata.json`. **Honest boundary:** the twin `ReaderCropContinuousTrial` is **not** accepted — it fails at its discovery step on the same gallery (`ReaderCrop sample=3 detected=false`, `Tests run: 1, Failure: 1`), and a direct probe opening the lab in continuous mode at the page index the spread trial found croppable (4) emits only `rkit-continuous-list` / `rkit-continuous-page-4` with **no** `rkit-cropped-image-*` node, whereas the paged/spread surface does emit it. So on this evidence the shared continuous surface does not present the cropped-image element that the trial (and the accepted NextE twin, whose gallery is border-croppable) expects. This is recorded as an unresolved continuous-crop boundary, not a pass and not yet a defect claim; a speculative trial edit that assumed the id was paged-only was tried and **reverted** because the run still failed, so no trial or product change was shipped. Trials next: determine whether continuous crop is expected to apply at all on NextN (legacy `cropBordersContinuous` exists in `ReaderPresentationState`) and, if so, why the shared continuous frame does not mark the crop as applied. Artifacts: `.hvigor/outputs/nextn-continuous-crop-f450aad/{run1,run2}/`, `.hvigor/outputs/nextn-crop-probe3/run1/dump.json`.
