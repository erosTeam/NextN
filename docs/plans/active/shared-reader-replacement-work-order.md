...
- NextN **runtime crop on/off via the shared menu (`ReaderCropToggleTrial`)**: **cannot run as written** on the NextN lab route, and no speculative rewrite is shipped. The trial launches the lab with `readerLabCropBorders='false'` and then expects the shared `rkit-crop-toggle` inside the runtime menu, but the shared chrome only renders that item when `cropAvailable` is true, and the NextN lab sets `cropPolicy.available` from `initialCropBorders() || labRequest.cropBorders === true` (`NextNReaderLabPage.ets:830-831`), i.e. it is **false** when the lab request has crop off and the request is not a production source. So on this route the toggle is absent by construction and the trial's `expect(toggle !== null)` fails (run 2026-09-18: `Tests run: 1, Failure: 1` at the toggle lookup). This is a **trial/lab-contract mismatch, not a product defect**: production sets `cropAvailable` independently of the current crop state (the Koma shared route already proves the real user control toggles crop on/off over real content — see the record below), and the NextN in-session crop **geometry** path is separately accepted through `ReaderCropTrial`/`ReaderCropSpreadTrial`/`ReaderCropZoomToggleTrial`. A correct fix would either launch the lab with crop on and assert the off->on->off toggle through the shared menu, or make the lab expose `cropAvailable` independently; neither is assumed here. Artifact: `.hvigor/outputs/nextn-crop-toggle-f450aad/run1/run-metadata.json` (failed, retained as a counterexample).
