...
2026-09-08 当前优先项（取代下方旧借窗状态）：共享诊断检查点3925f45d069f53b9a7b6050384e889e04882b70d clean，142真实模块测试通过；Koma本次单包构建窗口已明确归还。E main12b/native12在237的108记录恢复成功/黑色属性回读早于host清除；同场107首Grid帧865仍白字、866才黑，root和NH均亲看，因此此边界仍failed QA，不能归因恢复失败，也不能宣称平台内部根因已明。104未复现不抵消107。下一动作先评估窗口/导航渲染提交所有者的最小同步候选，避免固定毫秒/帧数延时；RTL测试准备保留但暂不切换，生产阅读器不替换。237租约已释放；197未触碰。E诊断与native改动保留WIP，未擅自提交E。
