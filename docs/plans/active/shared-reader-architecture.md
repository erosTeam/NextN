...
当前下一动作：103正常文件夹导入和正文路径分别以 FileUri.path 最小修复，多章样本已观察到共享阅读器前后切章及横竖屏正文、生产阅读器正文；但撤回无依据 rail 候选后的恢复包35同页正文失败，不能放行重启可靠性。先读取既有权限激活日志，区分宿主准备/权限与实际源路径，不以重新选择目录掩盖失败。之后核对章节准备失败、关闭/后台期间在途取消的真实异步边界；同步本地切章不能替代该证据。完整设置/旧进度衔接及三宿主剩余矩阵仍 OPEN。当前任务按小时续跑，调度不等于验收完成。
