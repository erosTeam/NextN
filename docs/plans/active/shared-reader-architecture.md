...
## 最新切片：D9 当前图片分享端口 — 2026-09-07（限域验收）

- 仅扩展可选 ReaderSurface 的当前图片分享叶子。沿用已映射的 E/N 顶栏父树：独立居中页码、56vp 行、44vp 控件/22vp 图标；分享插在设置之前，不移动图像、进度或缩略图栏。不新增重复作品分享菜单，不迁移保存/裁切/翻译和持久偏好。
- 请求从当前 snapshot.anchor.sourceIndexHint 与 unit 的副本确定原页，和顶栏页码同源；不能取 RTL visual frames[0]，不能把物理半页误当另一原页。携带导航代次，准备完成后再次核对同一单元/原页/代次和 active，旧请求不迟到显示。
- 中性端口只描述当前图片动作和准备/展示生命周期；网站 URL、缓存文件、ShareKit 数据记录及 Koma 章节均留在宿主边界。N 使用自身原图文件/链接语义，E 使用现有正常图像 URL 语义。准备失败不得冒充图片分享成功，也不静默改为分享作品。
- 官方 ShareController.show 是显示面板的 Promise；on('dismiss') 才是面板关闭事件。先注册 exact callback，展示期间保持 busy，关闭或显示失败后清理同一回调；不把显示成功或关闭当成用户发送成功。准备期取消与已展示面板的生命周期分开，避免后台切换误释放仍在使用的资源。设备验收只打开/取消，不选外部接收者。
- 已完成86项真实core测试，新增5项覆盖迟到结果、新旧请求交错、重复点击、后台/卸载后已展示资源保留、准备/展示失败与再次请求。N最终main3/native2签名构建10s323ms/8s807ms，E main2/native2为12s928ms/13s173ms；Nmain2类型收窄失败未安装，记录保留。此前E01详情P117是历史，E02因审批流adapter_eof未执行，不算设备失败；跨日03–08重新建立当前P2参考。
- 237实机8项原生流程各1pass/0failure/0error，均逐张检查全部整屏PNG/native根：N09/E10正常P2分享/取消→P3再次分享/取消；N11/E14准备失败→实际提示→同页重试；N12/E15准备延迟时真实换P3→迟到不弹窗→新页分享；N13/E16准备中退出→迟到仍普通宿主→新试用P2分享。E10双页原图[0,858][660,1261]/[660,858][1320,1261]与E06/08同页参考一致。N提供FILE、E提供HYPERLINK记录，标题对应当前原页；全程未选择接收者。
- 失败/延迟为显式Debug探针：先准备真实宿主数据，再只注入一次失败或3s迟到，不改网络/缓存内容，不据此声称任意传输取消已验证。只冻结上述端点；连续/RTL/物理半页分享组合、Koma分享适配、其他宿主动作、持久迁移与最终替代仍OPEN。下一步保存共享/N独立检查点，继续映射并接入下一项缺失宿主动作，不重新改动已接受的主图几何。
- 临时构建窗口已结束：Koma 明确归还后，本任务复查 reader-kit 仍 clean 2063b3b。允许继续 D9 实施，未做 stash。跨日237新租约为20260907-023744-59bf3baa；03唤醒发现USB对话框，04保留仅充电关闭它，实际NextE详情当前进度P2，不擅自恢复历史117。05/06已实看正式P2原图与chrome；后续同页对照改为当前P2。

## D8 宿主输入端口 — 2026-09-06（历史限域验收）
