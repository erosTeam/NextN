...
| D11 NextE整页覆盖声明 | root匹配当前共享候选；NH子代理已复核实际sprite producer和可参数化原生入口 | 当前声明尚未实机验收；缺已确认适合录屏的完整sprite样本。现有readerLabWork/readerLabUnit可选真实作品；非sprite Cover必须继续unknown，不继承旧main9结论 |
| D11退出系统栏时序 | NH子代理共享异步窗口队列；双页子代理E宿主及86完整帧复核；root N宿主/真实方法测试/匹配构建与237 | Nmain22/native29的87双页、88普通入口、89可见等待Back、90隐藏等待Back限域接受；86返回336白字Reader→337深字Detail，独立完整复核已回收，无录制空黑/跳位。E main11/native10构建成功但未实机，不加猜测帧延迟 |
