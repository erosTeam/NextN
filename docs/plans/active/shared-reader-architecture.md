...
| D11 NextN未知覆盖预览交接 | D12当前切片完成后由root主责；NH子代理给出基于已保留运动反例的最小候选，双页子代理交叉审查目标页布局 | unknown恢复旧淡出仅限域通过；裁切预览与原图错位叠影仍OPEN，不凭比例猜原图crop，不将NextE整页策略全局化 |
| D11 NextE整页覆盖声明 | root匹配已构建候选与当前设备，子代理复核完整运动 | main10有效sprite声明尚未实机验收；非sprite Cover必须继续unknown，不继承旧main9的通过结论 |
| D11退出系统栏时序 | 单独小切片，root统一宿主退出所有者；NH子代理已交只读原因和最小方案 | N/E已有一帧白字Detail反例；需恢复完成与退栈的实际时序验证，不加猜测帧延迟 |

两项缩略图研究均是阅读器主线输入，不是已交报告即可关闭的旁支。上述队列不代表同时编辑四处：D12限域端点复核已收束，当前只实施D11 unknown交接；后续项保留明确反例与下一动作，每项由root复核子代理交付并完成实机验收。Koma真实样本已在本地产物找到10张Solo Leveling下载页，仅经普通导入使用，不伪造库记录或用生成测试图。章节编排/整作完成仍由宿主所有，后续Koma章节接入须在本切片限域验证之后推进。
