...
1. **D10 收尾（root）**：完成既有图片信息后台异常路径；定位并修正 E 同页信息原图可用性误报，保存窄范围检查点。不扩展其他菜单叶子。当前 E20 关闭取消端点已观察，但新会话 P2 原图可用性由不可用变为可用，字段语义重新 OPEN。
...
分工边界：`assess_spread_transition` 输出 UI 布局/交接最小接口、文件归属和先后依赖；`assess_nh_thumbnail` 输出 Detail/Grid/rail 的预览语义、已知/未知对应关系及最小等比候选。两者只读且不操作设备，不重复审计。root 合并为同一协议后再拆互不重叠的实现工作，负责宿主接线、237验证和检查点。Koma保留中性接入能力，章节切换继续由宿主拥有，不拿编译代替其运行验收。
