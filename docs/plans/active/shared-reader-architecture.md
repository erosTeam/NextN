...
- **交互组合**：197与103覆盖横竖屏；单页/双页/连续、LTR/RTL、普通/长/宽图覆盖其适用行为。缩放必须实际拖动并检查最终内容位移，连续模式检查图片与列表滚动归属；点击、滑块、菜单、重试不得被阅读手势吞掉。旋转检查页/片段/缩放锚点，不只看无裁切。
...
当前下一动作：追查103正常文件夹导入失败，只有源代码/实际异常能定位原因才做最小修复，然后通过正常导入建立多章平板样本并验证跨章；既有单章CBZ和197章节证据不能替代此项。并保留准备失败、在途取消、完整设置/进度衔接等OPEN项。当前任务按小时续跑，调度不等于验收完成。
