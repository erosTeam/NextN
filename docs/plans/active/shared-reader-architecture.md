...
| 能力边界 | 必须保留的旧阅读器能力 | 最低验收证据 |
| --- | --- | --- |
| 入口与退出 | Detail、全部缩略图、Reader 缩略栏的真实页身份；进入、立即退出、返回原位置；未知局部缩略图不冒充整页 | 同入口同页的连续转场原视频和终点原图；真实 host/root/viewport；返回源页面位置不变 |
| 图片资源生命周期 | 原图/缩略图选择、预加载、慢载、逐页失败、精确重试、取消、旧请求退休；成功邻页不被失败页重载 | slot/request/asset 身份日志 + 实际正文/错误层 + 重试后同页；失败、退出、翻页、旋转各有命名反例 |
| 布局模式 | N/E 的横向 LTR/RTL、单页/拼合/均分/封面与末页；Koma 额外连续/竖翻、章节边界；条漫不可被视口高度裁切 | 手机197与平板103的真实页顺序、图片边界和导航；横竖屏重排；长图实际滚动位移，不以标签替代正文 |
| 手势与缩放 | 保存的点按区域/反转、滑动方向、双击、双指、放大后拖动；连续模式放大时图片与 List 仲裁；音量键 | 实际输入后的页/坐标/scale/scroll 变化；菜单/错误卡/缩略栏不误触；缩放与前后台、旋转、资源替换组合 |
| Chrome 与动作 | 顶底栏页码、进度条、模式/双页/缩略图、原图/保存/分享/更多及各宿主实际支持项；短按钮和失败提示维持已接受视觉 | 隐藏/显示/重显、动作结果和不可用态；同屏原图/layout；宿主不支持的叶明确分类而非静默删减 |
| 设置承接 | 三端现有模式、双页、奇偶、裁边、页码、背景、图片插值、自动翻页、常亮、点击区、音量键等 canonical 状态 | 从正常 UI 改值并进入可选阅读器，或同步敏感宿主只读；实际叶/行为证明生效；原类型、存在性和值精确恢复 |
| 进度与章节 | 原阅读进度、冷启动恢复、返回详情/书架一致；Koma 上下章切换、准备失败保留旧章、取消与末章语义 | 不直写进度 fixture 的真实路径；进程重启前后；Koma 真实多章样本；准备失败/取消不清空可读旧章 |
| 生命周期与适配 | 前后台、关闭中、快速重开、旋转/窗口变化、手机/平板；不会由迟到回调复活旧页或丢失当前位置 | 当前候选匹配包的连续动作、窗口/根身份、关键 revision/position；197/103 各自恢复方向、超时、音量和设置 |
| 数据与回退 | 默认旧阅读器入口、设置/进度 owner、下载/书库/同步数据均不迁移；可选入口随时关闭 | 安装前后业务数据签名或精确字段比对；默认入口未改的源码审查；退出后正常旧宿主可用 |

状态记录必须按 **SOURCE / BUILD / NATIVE / DEVICE / VISUAL** 五层分别记载；`LIMITED` 只能覆盖明确列出的宿主、模式、样本和设备。任一层出现反例时，该能力保持 **OPEN**，不得由其他模式的通过结果投票抵消。已接受且源码未变的边界冻结，不为增加数量重复执行。

2026-09-14 当前收束：NextN 完整下载的 immutable `file://` 同 URI 重载已在103分别闭合分页单页、连续模式和双页 SPLIT。最新双页 runs01/02 均从真实 Downloads 路径打开 gallery661990 的来源38/39，逐来源重载后保留两张已解码图和39/47，且精确恢复队列、八列历史、三项阅读设置与 legacy backend。该结果只关闭完整本地来源生命周期的三种已命名布局，不替代损坏文件/缓存驱逐、旋转中重载、NextE/Koma消费或默认替换门槛。下一执行仍从完整替代账本选未闭合的宿主能力，不把本次设备通过当成目标结束。
...
2026-09-08 当前检查点：共享3925f45d069f53b9a7b6050384e889e04882b70d freshly clean；Koma候选二构建窗口已归还。HDS-only110与close-stack隔离113均复现首Grid白字，已撤回；107/108恢复成功与屏幕不同步的反例仍OPEN，不猜内部根因。main15/native16已在237完成Grid117和Detail120的RTL命名路径，全部6整屏与独立连续录像116/119（979/1008帧）核对：P1直达右页、P2左页随后出现、原图原位接替，返回各自源位置。两次首返回页字色正常只代表本次，不抵消旧反例。冻结这些RTL命名运动；下一未验证物理边界是可选入口视口变化/旋转取消，待237调试效率试点归还后执行。237租约已释放，197未触碰；不替换默认阅读器、不迁移设置/进度、不擅自提交E。

当前结论（97/99原帧复核）：Detail健康LTR本样本限域通过；Grid完整链仍failed QA，首返回帧841白状态字、842深色。下一片由root沿现有退出所有者记录真实save/restore属性与host清除时刻，先区分平台成功/属性回读/屏幕采用，不用猜测延时。共享21f90808cb32abbe5ed3fd7572a09a119078d584已实时确认clean，并向Koma章节多选任务放行一次assemble+独立包复制窗口，归还前不修改共享/触发消费者构建；实验两文件继续冻结。

2026-09-08 Non-H续跑：用户指出直接使用分类后，237原生91–95已发现并核实4175844（Non-H、46页、真实公共gallery token）。此前“需用户提供链接”不再是阻塞，以下历史进度中的未安装/缺样本已由96–100取代。96安装当前E main11/native10；98 Detail原生1pass22.886s、100 Grid原生1pass22.671s。root检查全部6张端点及各自根[0,120][1320,2120]：Detail P1[72,1214][391,1664]、Grid P1[37,309][435,869]返回均未动；两条目标P1[0,594][660,1526]/P2[660,594][1320,1526]。97/99连续运动独立复核中，不将原生通过替代动效验收。237租约已释放，197仍Koma任务独占；共享21f9080当前clean，Koma两实验文件冻结。

| 边界 | 责任与下一动作 | 通过条件 / 当前未通过项 |
| --- | --- | --- |
| D12 Koma 可选共享界面（限域收束） | root已完成237真实路径，双页子代理独立复核13–18整屏；NH子代理生命周期复核无阻断 | main2已安装，默认/旧Lab保留，错误页两种退出、正常导入10真图、P4→P5/前后台/双页/连续滚动/返回及库字节不变已观察；独立视觉复核已回收，限域端点冻结；Koma两文件增量未提交，不动生产Reader与197 |
| D11 NextN未知覆盖预览交接 | NH子代理实现两文件候选；root审diff/匹配包/237，双页子代理独立复核77/83完整录制帧；保存限域检查点 | main21/native28：单页与健康LTR双页交接无录制帧错位叠影/空页，左右原图失败重试、等待中翻页通过命名路径；保留明显离散替换，不宣称无缝映射或RTL通过 |
| D11 NextE整页覆盖声明 | root完成237 Non-H真实样本及98/100原生路径；双页/NH子代理分别复核97/99连续帧 | main11/native10当前包：Detail及Grid均同epoch captured/moving/finished/closed，P1/P2完成、原源位置保留；完整运动复核中。非sprite Cover继续unknown，不继承旧main9结论 |
| D11退出系统栏时序 | root拥有下一条Grid实际恢复时序诊断；NH子代理独立复核99及await链路，双页子代理复核97 | E Grid99 failed QA：841首浅色Grid仍白状态字，842才深色；100原生通过不抵消。97 Detail822→823首Detail已深色。shared setter Promise仅API settle，不能宣称合成已采用；保留反例，不加猜测延时 |

两项缩略图研究均是阅读器主线输入，不是已交报告即可关闭的旁支。上述队列不代表同时编辑四处：D12限域端点复核已收束，当前保存D11 unknown交接命名路径；NextE样本前提未满足期间推进已保留反例的退出系统栏时序，不伪造E通过。后续项保留明确反例与下一动作，每项由root复核子代理交付并完成实机验收。Koma真实样本已在本地产物找到10张Solo Leveling下载页，仅经普通导入使用，不伪造库记录或用生成测试图。章节编排/整作完成仍由宿主所有，后续Koma章节接入须在本切片限域验证之后推进。

当前复核收束：77全部772帧/22联系表、83全部549帧/16联系表及关键原帧由独立子代理审阅，root已核关键整帧与实际根。83预览等比飞向左P1，右P2独立原位出现；308→309离散换成原图，无中间空页/错位预览；309原图上仍有一个录制帧的加载环，310消失，不宣称同帧清除。84当前Window545根[0,120][1320,2120]，不同于78的Window541；79–81根[0,117]，不跨viewport作像素比较。新77/83首个完整Detail均已深色状态栏，旧74/E67反例保留，未修改退出所有者。当前共享133项真实方法测试通过仅用于状态/所有权回归，不作视觉证明；默认阅读器和持久化均不改。

退出时序实现进度（2026-09-08）：shared close返回可重复await的同一Promise，平台操作跨实例排队；N/E先撤销window资格，保持可见Reader到restore settle才清同request/epoch，hidden取消仍同步。新Want等待旧关闭后重验latest/lifecycle，初次foreground=false不会被新增guard丢弃。实际方法测试shared140、Nhost9、Ehost9通过，非UI静态契约。Nmain22/native29构建10s857ms/7s604ms，E main11/native10构建12s656ms/11s538ms；E未安装/未实机，不继承旧包。当前237 N87 Window549 root[0,120]，88–90同Window549 root[0,117]，各链独立审阅，原生分别1pass16s588ms/7s330ms/23s777ms/21s542ms。86录像原生编码1196x1920、531帧；不与1320x2120 PNG混用像素坐标。root已核336/337原帧及88普通/89visible/90hidden整屏终点，独立复核下一步。NextE实际完整sprite与退出路径缺已确认非成人样本，已询问链接；该前提不能转成通过，默认替代和Koma章节仍OPEN。

当前进度（23:21）：D12独立复核与root一致，命名端点限域冻结；Koma源码两文件增量仍单独保留。D11选择unknown落位/解码后显式零时长同步reveal/finish，不再将裁切图与原图交叉混合；NH子代理只编辑presenter和真实方法测试，root负责官方API核对、diff、N匹配构建/237运动，双页子代理复核新连续帧。280ms飞行/whole-page140ms/邻页到达/host不改；这是有待实拍取舍的离散内容替换，不是无缝转场通过结论。

当前进度（22:38）：共享c4c6350与NextN证据2e886da已保存。Koma DL004消费一次clean构建并归还窗口，197继续其固定包QA，不再等待研究结论。D12正式进入可选chrome/退出接入：237新基线确为空书架，无库/下载队列；root负责Index onClose、现地样本/构建/验收，NH子代理只编辑KomaReaderLabPage复用NextE可选父树和共享ReaderSurface，双页子代理保留D11运动复核结论。旧Lab和生产Reader不动，adapter/章节完成/持久化不动；正向阅读必须有真实237样本，空库不通过。N74已恢复旧unknown淡出，错位叠影与一帧状态栏色仍OPEN；E main10当前源声明另待新实机，不被Koma切片吞掉。
...
当前进度（19:26）：39只增加原生只读状态日志，确认smartGrip/support=true下edge在首次横屏从right变left；FAB按该状态落位，未确认坐标回归，故不改产品按钮或用户设置。39原生1pass15s859ms及7张整屏/current root已审阅。下一实际依赖为NextE源端：复用裁切后content快照，但必须贯通真实sprite就绪状态、保留已有导航拦截所有者；当前适配器原图尺寸为0，不能用thumb尺寸替代。双页子代理继续只读收敛可信解码/原图头尺寸进入现有布局的最小契约，root审查并安排可选接线实施。37原始疑点及39区分证据都保留；不重跑已冻结普通等待/Back端点。

当前进度（19:20）：共享688059e已保存已验证的等待反馈层级，35/36等待释放与Back、38连续501帧限域接受；37真实旋转的同entry取消和原图交付通过，但返回同竖屏后详情浮动阅读按钮从右下跑到左下，整页返回仍OPEN。NH子代理负责只读追踪这个宿主布局回归及最小区分测试；双页子代理只读准备NextE真实sprite快照到可选共享入口的精确接线清单；root负责237证据审阅、取消/交付集成与下一修复。Koma已消费并归还clean688059e单次构建窗口，197继续其独立QA。后续普通/宽图、NextE源端和Koma宿主边界仍按同一D11队列推进，不因研究结果交回而搁置，也不提前替换生产默认。

用户纠正：双页转场和 NextN 条漫缩略图属于阅读器主线，独立评估必须转成实施安排，不能只作为旁支存档。以下顺序取代旧记录中“继续下一项菜单动作”的泛化下一步；默认阅读器仍不替换。

1. **D10 已限域收尾（root）**：N14–17/E18–21异常端点及 E23 首次/重开 P2 原图可用性已实机观察。E23匹配main7/native5，原生1pass/0failure，六张完整图与root已检查；真实resolver回归5/5。保留未覆盖组合，不扩展其他菜单叶子。
2. **D11a 转场数据与布局接口（root 集成，双页/条漫子代理提供落地设计）**：复用 core 的 unit/source/fragment 身份，reader-ui 发布实际呈现内容矩形、布局代次和就绪状态；宿主提供实际缩略图快照、源矩形和预览对应关系。未知 crop 不由比例猜测；PixelMap、导航和章节编排不进 core。
...
- **首版不假设 NH 普通页是 whole**。普通/条漫均在没有对应证据时标 unknown；比例相同不是升级条件。Detail 提供 Cover 后真实可见预览及裁剪框，Grid 提供 Contain 实际图像矩形，不把 tile 留白算图像；sprite offset 只描述从精灵图取预览。
...
用户明确“单独处理”是不打断共享主线，授权子代理分别评估。以下源码评估现已进入上述 D11 实施队列；它不是设备验收，具体交接动效仍需可选接入后的连续画面决定，淡入不是先验定案。

- 双页入场：两端 `ReaderThumbnailTransitionState.arm` 都先用整屏单图 contain 终点，实际双页内容由阅读器拼合/均分布局决定。N `ReaderSpreadImageLayer` 还缺少单页及 E 双页已有的 `readerImageOpacity` 交接门。单改终点不能修完显隐交接。建议候选由阅读器真实布局发布被点中源页的内容矩形，代理只连接起点和该矩形，邻页原位晚段淡入；RTL、封面/尾页独页、异比例、裁边、失败、旋转均使用同一源页/fragment/布局代次。参数留待连续录屏比较。N 普通双页退出目前明确不走自定义 close，不能把新增返程行为混入入场修补。
- NH 预览：API分别解析原图尺寸、thumbnail URL/尺寸，当前DTO没有可信原图裁切矩形。Detail的150vp高横栏仍依原图比例或0.65fallback定框并Cover，可能二次裁切；Grid当前已有未提交的解码比例优先/Contain修补；旧Reader栏thumbnail DTO优先/解码后备/Contain。这三处各自正确与否不可互相代替。
- 当前转场抓取的是实际节点快照（包含客户端裁切或占位框），N root及Reader-owned opening proxy却Fill到原图比例终点。这是独立于Grid修补的拉伸风险；没有crop mapping不能凭尺寸反推原图位置。E的sprite偏移仅定位预览在精灵图中的区域，不自动证明原图映射。是否完整展示源站预览、是否保留整齐且可裁切的Detail框、是否由已有原图生成整条概览，是不同产品取舍，未要求用户现在决定。
- 可比较“独立预览等比运动＋明确图层交接”与“页面级展开/揭示”；不能伪称局部预览是完整原图。仅可信crop mapping或同原图确定性生成预览时才做真实局部到整图展开。共享core保留源页/配对/fragment；reader-ui拥有实际内容矩形与呈现就绪；宿主拥有起始快照、导航和系统栏。
...
- 异常路径检查点共享 `31d6be5`，仅显式 Debug 的 missing-header / delayed metadata 探针，默认未启用路径不变。N14–17 / E18–21 八条原生流程均各1pass/0failure/0error；所需完整PNG与当前root已检查。E21后台返回19s362ms，保留P2、拒绝旧弹窗且可新请求。仅冻结异常状态端点，不接受原图可用性字段。
- E20/E21同P2文件均WEBP122.1KB/1280x782，但原图可用性true/false。实际 `ImageResolveService.resolve` 合并请求只返回string，未给joined caller应用metadata；实验adapter把未同步的空originImageUrl误作false。新增直接执行真实resolver/parser的受控IO回归，修改前4项中3项失败：joined调用者四字段均空；失败后重试原本通过。最小修复是让同一flight携带自己完整且独立的解析结果，每个consumer应用同一结果，不能读取可能已被换源替换的全局cache来补。源代次/cache策略、下载、界面布局与原图选项均不改。需修复后实际回归、E main/test成对构建及237同页首次/重开信息对照。

- 已接入候选：实际asset绑定只读metadata source，按需读取本地文件ImageSource头；源页DTO和缩略图比例不作为文件格式/尺寸。核心隔离slot/request/unit/navigation/前后台代次，读取失败不改变图片displayed状态。菜单固定在参考more位置，保留视觉左/右原页选择，系统AlertDialog显示当前文件事实；不迁移默认阅读器或设置/进度。
- 普通信息路径限域验收：最终N main2/native2构建11s214ms/9s430ms，E main5/native3构建12s751ms/19s813ms；N13/E12各原生1pass、0failure、0error，25s357ms/24s107ms。20张完整截图和各自EntryAbility根[0,117][1320,2120]已逐张检查。N源图P1/P2为720x9245/720x9980；E两页均1280x782，P1/P2大小110.3KB/122.1KB，原图可用性各自保留。系统信息弹窗、视觉左右页选择、消失后恢复原页、继续P3和退出普通宿主均已观察。更多图标不再在已显示的弹窗背后转圈。
- E首次原生失败不是图片坏了：测试包/主包rkit_unknown数字ID不同，原helper把测试包ID交给主包resourceManager。改为HAR宿主已合并资源的稳定名称访问，直接组件资源保持原样；必须成对重建main/test再验收。92项实际core测试通过；原生读取失败/迟到、连续/物理半页、Koma信息接入仍OPEN。先保存普通能力检查点，协调NextE多选→Koma下载单次clean消费窗口后继续异常路径，不切默认入口。
...
设备计划（2026-09-12 当前用户指令）：仅使用197手机和103平板，237由用户使用，禁止占用。具体使用仍需实时目标、内容条件及协议。跨应用参考对照需要同状态、同有效视口；不同设备截图不直接充当像素或几何对齐证据。

### 10.1 自主推进与替代放行标准（2026-09-12）

用户要求长期自主推进并防止功能退化。常规前置修复与已授权验证自主执行；只有实际产品取舍、新的数据破坏风险或超出授权范围才请求决定。保持可选接入，最终替换仍须单独放行。
...
当前下一动作：缩略图检查点共享7d892fa/NextN8413c9c已保存，280ms转场动画因录像导出失败仍OPEN。自定义点击区只读承接完成三端源码与构建、299/299真实方法测试及1404宿主解析对照；197根审N06 L形分页/2x菜单/连续2040px来回与六项设置恢复、E09当前只读分页/菜单/退出，103根审Koma横竖屏L形分页/75%连续滚动/2x菜单与22项设置和库哈希恢复。只接受命名路径，不推广到全预设、真实失败重试或整体替代。详见shared-reader-tap-zones-20260912.md。下一实施前审计为隐藏工具栏时的页码偏好承接：保留工具栏导航页码，复用三宿主既有canonical状态，不新增恢复或写回；背景/缩放质量/页间距/动画分别留队。E开启同步保持只读现值。Koma真实双指、裁边/失败/转场组合、章节准备失败、实际在途取消和旧进度完整衔接仍OPEN。默认替换、设置/进度写回不启动；仅197/103，按小时续跑，调度不等于验收完成。
...
| --- | --- | --- |
| NextN | ReaderPresentationService/State.volumeKeyTurn；只读恢复与 nullable debug override，未写回；9fc57d8/shared366fa17 | 197 native9430 四组合、系统音量恢复、关闭归还已接受；仅系统事件注入，不是人工机械按键或所有设置一致性 |
| NextE | ReadModeSettings 实际发布 volumeKeyTurn 后标 runtime ready；Lab 继承或显式覆盖并处理关闭；无重复 restore | main40062/native69648 在197四组合1pass/42.466s，系统归还及原值恢复已接受；方法测试覆盖恢复失败，非实际I/O失败设备证明；无E提交 |
| Koma | ReaderPreferencesStore/ReaderModeState.hydrated；Lab启用与反向映射由宿主读取，共享6009a1b仅接收布尔方向，关闭同步释放；不写偏好 | 103 Koma52832：正常/反向、继承启用禁用、显式覆盖、关闭系统音量归还接受；原false/default/音量9/超时10000及库/进度hash恢复。动态映射/迟到回调仅方法证据，不是后台实测；无Koma提交 |

共享6009a1b的默认映射回归：197 NextN main6301/native9210配对1pass39.992s，NextE main90320/native9315配对1pass40.934s；原偏好/音量恢复，两设备租约已释放。初次NextN漏主包安装和Koma11/12无效退出路线不计入验收，原始证据保留。命名输入路径冻结，不扩大为全模式/完整视觉通过。
...
- **2026-09-19 收敛（此前的“当前共享policy仅single/spread/continuous”已过时）**：该两项能力此后已实现。`ReaderDisplayPolicy` 现含 `pagingAxis: 'horizontal' | 'vertical'`（纵向分页轴，`ReaderDisplayMap.ets:12`，由 `ReaderPagerSurface` `.vertical(...)` 消费、`ReaderPagedSession:832` 纳入签名）与 `spreadLayout: 'joined' | 'split'`（`ReaderDisplayMap.ets:7,13`，等宽 SPLIT 双页几何，非 `splitWidePages` 宽图裁片）；运行时菜单 `ReaderChrome.ets:409,413-415` 暴露 `rkit-vertical-paged` 与 `rkit-spread-layout`。两项均在当前 pin 有真机记录（详见工作单 P4/P5 及 `docs/qa/shared-reader-vertical-paging-20260912.md`、`shared-reader-split-layout-20260912.md`）。原句保留为历史：当时确实两者皆无。
