...
> **反例失效声明（2026-08-22）：** 本文件原先把 D/E/F 写成
> `AUDITED / VERIFIED / 已完成`，这个完成结论已被当前源码反例推翻，不能继续作为
> “NextE 移植删减已经检查完”的证据。`GalleryMediumCard` 从基线起就在同一加载态
> 同时构建图片占位符和进度指示器；六种图库卡片各自复制图片加载状态并遗漏 NextE
> `EhThumbnail` 的 `OPACITY` 内容过渡；`PullRefreshWaterFlowScaffold` 从基线起虚构
> 两列首帧宽度，而当前 NextE 明确使用“实测宽度 -> 同窗口共享宽度 -> 实时屏宽”。
> 这些都没有被当时的能力存在性脚本、稳定态滚动观察或页面入口盘点发现。因此本文件
> 只保留历史经过；D/E/F 的全局完成表述全部降级为**当时已检查的有限样本**。
> 新审查不得用脚本通过、构建通过、入口存在或稳定态截图推导组件语义完整。
...
结论：本次没有发现可由“抽取一个公共核心”安全消除的已证实功能缺失；当前副本差异承载
  ...
  ScrollScaffoldCore 重构。

D 的设备边界不是泛化的“NextE 视觉相同”：只验证每个 Scaffold 家族的代表性宿主是否
真的触发其契约——刷新开始/结束、滚动启停、到尾回调、Grid/WaterFlow 捏合密度和
WaterFlow near-end；List 不验证不存在的捏合能力。结果以状态树、回调/诊断标记和必要的
截图共同判断，不能用一张截图代替行为验收。

### D 运行时首轮状态（USB 56T0225315001128，2026-08-20）

- 普通 WaterFlow 滚动：VERIFIED（可见图库项 bounds 从初始集合移动到后续集合）。
- 到尾回调：当前条件无法测试；到底 fling 改变了可见内容，但本次捕获没有稳定的回调/请求诊断标记。
- 顶部下拉刷新开始/结束：当前条件无法测试；手势已执行，但没有可归因的开始/结束诊断标记，不能以最终布局代替刷新完成。
- 活跃根 Tab 重按：当前条件无法测试；测试时自动隐藏底栏不在目标坐标，未形成有效动作前置条件。
- Grid/WaterFlow 捏合密度：当前条件无法测试；本机 `uitest uiInput` 帮助未提供 pinch 注入，且 USB 设备未安装 NextE 供同设备对照。
- 证据已写入 `docs/qa/nextn-active-acceptance.md` 的同名 USB 记录；已通过的普通滚动不再重复验证。
- 一次定向“手势后立即取 app 日志 + 最终布局”复核仍未得到刷新/到尾标记；因此按“当前条件无法测试”收口，后续不再盲目重复，只有已有稳定诊断事件或明确授权的诊断改动才重新开启。
...
设备验收只覆盖 Reader 仍共享的模式/手势/图片边界；NH 专属翻译和数据叶不要求复制 NextE
视觉。验证应结合交互后的状态树、图片 bounds、加载/失败日志和必要截图，不能只看静态画面。
...
本批没有回加 EH 专属叶，也没有重排 NH 详情父树。保留的 OPEN 项是设备状态验收：详情首次
进入、显式刷新、缓存命中/失效、相关画廊加载和下载状态转换仍需验证。其中只有详情页仍
共享的父树、加载占位和操作栏状态做 NextE 几何对照；NH 专属标签、种子/下载数据和内容
分支不要求“像 NextE”。需要同状态 layout bounds、交互后状态和日志结果，截图只作辅助。
...
按钮状态、快捷搜索与历史胶囊的布局和冷启动持久条件仍属于 OPEN 运行时验收项；只对搜索
输入、历史/快捷胶囊和筛选入口这些与 NextE 共享的父树做几何对照，NH 高级条件字段不做
NextE 视觉等价要求。输入焦点、条件持久化和请求结果必须用布局状态与交互结果验证，不能
只看静态截图。若发现具体回归，必须在 `SearchPage` 的同一父树内处理，不能只改一个文本框叶子。
...
- D：族内静态能力矩阵与机检 VERIFIED；代表性宿主的刷新/滚动/捏合行为仍需设备验收，
  不做泛化视觉 parity 或猜测性公共核心抽取。
- E：Reader、GalleryDetail、Search、Settings 首批源码映射与删减嫌疑收口已完成；只对
  共享父树的状态/几何做有边界的设备对照，NH 专属叶不纳入 NextE 等价验收。
...
本文件只能证明 A–F 中记载的具体搜索、脚本和有限样本曾经执行，不能证明源代码隐患或
NextE 移植删减已经完成审计。2026-08-22 的反例确认旧机制缺少两项必要覆盖：共享根组件
内部状态机的逐分支对照，以及首帧/加载/错误/复用/恢复等时间状态的运行时验收。因此
D/E/F 的“全量完成”结论作废；真实设备行为、共享父树几何和时序仍须按可枚举行为切片
分别关闭，任何一片关闭都不得外推为整页、整个组件族或全软件完成。

## 2026-08-22 审查机制纠偏（当前有效）

审查单位改为**可枚举行为切片**，不是文件、入口、文本命中或页面名称。每个切片必须同时
有以下四类记录，缺一即保持 `OPEN`：

1. **参考语义：** 标出 NextE 的完整父树、共享根组件和状态机；NH 只能替换有证据的
   数据/动作叶。不能以“本地已有类似组件”跳过逐分支对照。实现依据只取当前已修复
   参考终态；历史错误版本和修复过程不进入移植模板。
2. **全调用面：** 从共享根向上列出全部 consumer，再检查同一父树的所有 sibling。
   修一个卡片或一个 Scaffold 不得代表同族完成。
3. **时间状态：** 至少覆盖初始挂载、加载、成功、失败、复用/换源、 retained 切换首帧、
   配置变化和冷启动恢复中适用于该切片的状态；稳定终态不能代表这些过渡态。
4. **分级证据：** 源码映射只关闭源码疑点，签名构建只关闭可编译性，真机状态/交互关闭
   行为，同状态同视口参考对照关闭可见父树。任何低一级证据不得冒充高一级结论。

当前仓库与 NextE 在 `shared/feature/entry` 下有 204 个同相对路径 ArkTS 文件，其中 37 个
字节一致、167 个存在差异。167 不是 167 个 bug：它只是尚待按行为切片分类的原始差异池，
必须区分 NH 必要叶替换、结构等价实现、无依据删减和未验证过渡态。审查进度以后只报告
已分类的切片和未关闭状态，不再报告“全量检查完成”。本轮先处理高复用根：图库图片加载
组件、List/Grid/WaterFlow host、retained subtab 首帧；其他页面和状态链保持未审结论。
