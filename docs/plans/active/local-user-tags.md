...
> 状态：COMPLETE（Phase 1-5、§12.4-§12.5、标签翻译库详情正文更正、临时验收数据清理及 WebDAV“应用设置”移除均已通过设备 197/103 验收）
...
- EH 的权重范围为 `-99..99`；NextN 的本地规则只要求安全整数，不继承这一服务端范围限制，默认值为 `10`。
...
weight: number               // 安全整数，无产品层人为范围，默认 10
...
详情标签保留当前单击精确搜索；500 ms 长按沿用 NextE 的标签详情入口，打开一个由详情页
根节点持有的半模态。半模态标题区显示带 namespace 前缀的翻译、原始
`namespace:name` 和 NH 数字 ID，只保留两个控制动作：快捷切换显式 Hidden，以及打开
本地标签管理编辑器。NH 没有 EH 投票语义，不增加投票按钮。

半模态正文的产品对象是 **EhTagTranslation 本地翻译库中的标签资料**，不是本地用户
标签规则摘要。必须沿用 NextE 的完整数据与展示合同：翻译库导入时保留 `name`、`intro`、
`links`；打开详情后显示翻译名称、`简介`、可点击的 `链接`、按两列自适应排列的简介图片，
并包含加载、错误和空态。简介中的行内标签引用按翻译库继续翻译；图片按“标签简介图片”
持久化设置过滤，默认“隐藏 H 图片”。本地 Hidden、weight、color 只属于上方两个动作及
共享编辑器，绝不能替代翻译库详情正文。

管理动作与本地标签管理页复用同一个 `LocalUserTagEditorSheet`。Hidden、权重、默认颜色
均使用共享列表行/行内输入组件；权重行只显示标题和输入框，不增加说明副标题，也不继承
EH 的 `-99..99` 服务端限制。阈值编辑项及其现有说明保持不变。保存或删除时先关闭 Sheet，
等关闭动画完成后再提交本地持久化并刷新外层列表；不能用提前改写绑定值绕过退出动画。
...
### 12.5 Gallery 标签详情与快捷编辑验收

1. 详情标签单击仍进入该标签的精确搜索；500 ms 长按才打开标签详情半模态。
2. 半模态只显示 Hidden 与管理两个控制动作，并使用统一 API 26 material Sheet 配置。
3. 正文显示翻译库的简介、链接和经当前图片级别过滤后的图片；不得显示本地 Hidden、权重、
   颜色摘要。没有详情负载时显示空态，读取期间显示加载态。
4. 标签翻译设置页使用现有共享列表行和下拉菜单，持久化“全部隐藏 / 隐藏 H 图片 / 隐藏引起
   不适的图片 / 全部显示”四级简介图片范围；默认“隐藏 H 图片”。
5. 同一实现分别在横屏宽窗口和纵屏窄窗口验证系统解析出的半模态形态、遮罩、标题、动作及
   可滚动正文，并与 NextE 的同标签、同视口状态比较。
6. Hidden 可直接开关且不破坏既有权重/颜色；管理动作打开与本地标签页相同的共享编辑器。
7. 中文翻译子串可找到候选，但只有经 NH 目录精确校验并取得正数 ID 的标签可保存。
8. 已有规则显示带 namespace 前缀的翻译、原始身份和右侧 ID；右侧控件使用共享边距。
9. 权重接受安全整数且不设置产品层人为上下限；权重行不增加副标题，阈值说明保持现状。
10. 自定义颜色、权重和默认颜色开关保存后立即重绘，关闭动画结束后再提交；冷启动与重新打开
   编辑器仍恢复相同状态。
11. 验收产生的临时规则必须通过产品 UI 删除，最终阈值保持原值且管理页恢复空态。
...
设备本地所有权，只进入本地备份。最终候选 `7850bdc` 已原位安装到 197/103：两台同步总览
均无“应用设置”，当前进程的启动同步均成功且没有 `settings-tables` 事件。197 的产品路径
清理已上传，103 冷启动后显示阈值 `0`、`尚未设置本地标签`，临时 Subtab 也不再出现。
设备 103 随后完成 §12.5 的横竖屏标签详情、共享编辑器、中文翻译搜索、大整数权重、颜色
即时刷新/冷启动恢复和单击精确搜索验收；新增临时规则也已通过产品 UI 清除并恢复横屏。
本计划已完成，没有遗留的本功能验收动作。
...
- 临时时钟诊断源码已经移除。197 通过产品路径删除测试标签和临时 Subtab、把阈值恢复为
  `0`，随后 `home-subtabs` 与 `local-user-tags` 自动同步成功。103 拉取后冷启动，本地标签
  管理页显示阈值 `0`、`尚未设置本地标签`；此前首页布局也已不再出现 `标签同步验收`。
  证据位于 `final-cleanup-rules-197-20260902T1111/`、
  `final-cleanup-subtab-confirm-197-20260902T1128/`、
  `final-cleanup-sync-settled-197-20260902T1129/` 和
  `final-cleanup-verify-103-20260902T1120/`。

### 15.6 WebDAV“应用设置”移除验收

- 源码阶段提交为 `8a6c65c`，父级摘要补正提交为 `7850bdc`；六项既有静态合同继续通过，
  本地备份合同确认四张设置表仍由 `localData.settingsTables` 导出/恢复。最终签名构建为
  `BUILD SUCCESSFUL in 9 s 271 ms`。
- 197/103 均以 `install -r` 原位安装最终包，没有卸载或清数据。两台“存储”页摘要均为
  “同步历史、阅读进度、快捷搜索和屏蔽规则”，同步总览均只显示阅读进度、浏览历史、搜索
  历史、快捷搜索、自定义首页标签、本地用户标签、屏蔽规则七项，不再显示“应用设置”。
- 197 当前进程 `51121` 与 103 当前进程 `24139` 的启动同步都在 11:13 完成
  `webdav_scheduled_done ok=true`；保留数据集正常运行，两个当前进程日志中的
  `settings-tables` 事件计数均为 `0`。旧远端 manifest 项未主动删除。
- 设备证据位于 `.hvigor/outputs/remove-settings-sync/final-storage-197-20260902T1115/`、
  `final-overview-197-20260902T1118/` 及设备 103 对应的
  `remove-settings-sync/final-storage-103-20260902T1115/`、
  `final-overview-103-20260902T1118/`。

### 15.7 Gallery 标签详情与共享编辑器验收

> 2026-09-03 更正：本节只证明了 Sheet 入口、两项动作、编辑器及横竖屏宿主形态；当时正文
> 错误显示本地规则摘要，因此不能作为翻译库标签详情正文的验收证据。

- 最终候选仅在授权设备 `192.168.50.103:12345` 原位安装；未卸载、未清数据。横屏长按
  `标签:巨乳` 打开 API 26 material 标签详情 Sheet，只有 Hidden 与管理两个控制；Hidden
  开启和恢复关闭均即时生效，管理动作打开与本地标签页相同的共享编辑器。纵屏验证了底部
  半模态与嵌套编辑器；普通单击仍进入 `tag:\"big breasts\"` 精确搜索。
- 添加页使用共享建议服务。中文查询 `朝` 返回带 namespace 前缀翻译、原始身份和正数 NH
  ID 的候选；`character:aoi asahina #14982` 保存了权重 `100010` 与粉色自定义颜色，证明
  本地权重不受 EH `-99..99` 限制。权重行只有标题和输入框，阈值说明未修改。
- 保存录屏显示 Sheet 先执行退出动画，随后外层管理列表才出现粉色 `100010` 徽标。原位
  冷启动和同页重新打开均恢复 `角色:朝日奈葵`、原始身份、ID、权重、颜色和默认颜色关闭
  状态。测试规则 `#2937`、`#14982` 最终都通过产品 UI 删除；纵屏及恢复横屏后的管理页均
  显示阈值 `0` 与 `尚未设置本地标签`。
- 核心证据位于设备 103 的 `.hvigor/outputs/.../local-tag-detail/`：
  `save-animation-20260902T145100/`、`cold-manager-20260902T145800/`、
  `reopen-tag-corrected-20260902T150300/`、`gallery-long-press-20260902T150900/`、
  `gallery-manage-20260902T151400/`、`portrait-1600x2560/.../gallery-manage-20260902T151800/`、
  `single-tap-search-20260902T152000/`、`cleanup-confirmed-final-20260902T154000/` 和
  `cleanup-restored-20260902T154300/`。

### 15.8 标签翻译库详情正文更正（ACCEPTED ON 197）

- 数据链候选已扩展为 `EhTagTranslation raw -> name/intro/links -> RDB ->
  TagTranslationInfoService.lookupTagInfo -> intro/links/images`，旧 RDB 行通过 schema 26 增列。
- 旧版仅有译名的本地字典会被判定为“详情数据未入库”，即使发布版本号没变也会重新导入，
  避免升级后长期只显示空详情。
- NH 的 `tag` 同时对应翻译库的 `female/male/mixed/other`；若译名、去图后正文与链接一致，合并各命名空间的
  示例图（如 `big breasts`）；正文或链接真有冲突时不按插入顺序猜测。
- NextE `TagTranslationService.lookupTagInfo`、`TagInfoIntroImage`、`TagInfoBody`、`TagInfoSheet`
  与四级 `IntroImageMenu` 已逐项映射到 NextN；NH 只在命名空间合并和两项动作上保留产品差异。
  本地规则摘要已从正文移除；简介、链接、两列图片、加载、错误与空态沿用同一结构。
- 设备 103 持续离线后，用户明确改用 `192.168.50.197:12345`。最终签名候选原位安装且未清
  数据；当前翻译库为 `v7.27868.1 2026-09-03T00:13:34Z`、`44059` 行。最初 `big breasts`
  空详情暴露了 NH 合并命名空间冲突，不能算通过；依据当前原始库确认 female/male 的文字与
  链接一致、示例图不同后，候选改为只合并等价正文及全部唯一图片。
- 纵屏 `1260×2720`、默认“隐藏 H 图片”下，`巨乳 / tag:big breasts · #2937` 显示翻译库
  简介及行内 `超乳 (huge breasts)`，不显示本地 Hidden/weight/color 摘要，也不显示被过滤的
  R18 图片。证据在 `.hvigor/outputs/local-user-tags-translation-info/197/
  merged-content-portrait-20260903T0400/`。
- 设置页共享下拉实际显示“全部隐藏 / 隐藏 H 图片 / 隐藏引起不适的图片 / 全部显示”。临时
  选择第三档后，同一标签实际渲染两列 R18 示例图；随后行值已恢复“隐藏 H 图片”。证据在
  `image-menu-20260903T0416/`、`r18-content-20260903T0419/`、`restore-nonh-20260903T0426/`。
- 197 原有系统“旋转锁定”为开启。验收中临时关闭后，窗口真实变为 `2720×1260`，同一详情
  使用居中宽 Sheet；28 帧连续序列显示 Sheet 由小到大进入而非瞬间出现。最终已恢复
  `1260×2720` 和蓝色“旋转锁定”原状态。证据在 `landscape-gallery-20260903T0446/`、
  `landscape-long-press-20260903T0454/`、`restore-rotation-lock-20260903T0458/`。
- 最终合同脚本、四套资源 JSON、`git diff --check` 与签名构建均通过。所选标签的 `links`
  数据为空，因此设备页没有伪造链接；链接解析、展示与系统打开动作由与 NextE 对应的分支及
  静态合同覆盖。原 §15.7 已验收的两项动作、共享编辑器和单击搜索仍保持有效。
