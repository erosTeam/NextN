...
> 状态：COMPLETE（Phase 1-5、§12.4-§12.5、临时验收数据清理及 WebDAV“应用设置”移除均已通过设备 197/103 验收）
...
3. 同一实现分别在横屏宽窗口和纵屏窄窗口验证系统解析出的半模态形态、遮罩、标题和动作。
4. Hidden 可直接开关且不破坏既有权重/颜色；管理动作打开与本地标签页相同的共享编辑器。
5. 中文翻译子串可找到候选，但只有经 NH 目录精确校验并取得正数 ID 的标签可保存。
6. 已有规则显示带 namespace 前缀的翻译、原始身份和右侧 ID；右侧控件使用共享边距。
7. 权重接受安全整数且不设置产品层人为上下限；权重行不增加副标题，阈值说明保持现状。
8. 自定义颜色、权重和默认颜色开关保存后立即重绘，关闭动画结束后再提交；冷启动与重新打开
   编辑器仍恢复相同状态。
9. 验收产生的临时规则必须通过产品 UI 删除，最终阈值保持原值且管理页恢复空态。
