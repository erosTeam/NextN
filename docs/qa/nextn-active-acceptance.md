...
- OBSERVED, NOT CLOSED — a bounded fling toward the tail changed the visible gallery titles and produced a later WaterFlow layout, but no stable `onReachEnd`/page-request diagnostic marker was present in the captured log. This proves continued content movement, not the reach-end callback contract. Evidence: `.hvigor/outputs/latent-hazard-audit-20260820-usb/bottom.json`, `bottom-hilog.txt` (local-only).
- OPEN — top pull-refresh completion: the gesture was injected and the final layout was captured, but the current build emitted no retained refresh-start/refresh-end marker and the layout contained only the ordinary loading node. No refresh PASS is claimed. Evidence: `.hvigor/outputs/latent-hazard-audit-20260820-usb/pull.json`, `pull-hilog.txt` (local-only).
