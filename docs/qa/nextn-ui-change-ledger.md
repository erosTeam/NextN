...
- **Unresolved risk:** erosTeam/NextN 当前无任何 GitHub Release（API 返回空数组），因此发布日志对话框的版本轨道/Markdown 正文、打开发布页与正文链接跳转需在发布 v1.0.0 Release 后补充真机验收；GitHub 304 空缓存路径会在零 Release 期间记录一次缓存缺失告警（NextE 同语义）；平板/横屏对话框布局未单独验收。
