...
这些规则来自 INC-001/004/005/006/007 的复盘，是“稳定功能反复被改坏”的
直接约束。违反任一条即视为事故，必须先登记再修复。

1. **组件身份只允许稳定身份 + 实际渲染内容。**
   ForEach key 禁止包含 epoch、revision、请求代际、时间戳、随机数、
   位置索引或成员数；key 变化必须对应真实的身份或渲染内容变化。
2. **数据源禁止因“等待中”状态剔除成员或整组。**
   加载/翻译/刷新期间需要隐藏时，用 `Visibility.Hidden` 保留几何槽位；
   禁止改变数组结构后让 key 连带变化（整组卸载重装）。
3. **常驻壳层。** 页面区块的外壳（卡片、轨道、标题行）一旦存在就必须常驻，
   loading/empty/error 只能在壳内原位替换内容，禁止整块 `if` 卸载。
4. **几何单一来源。** 间距、指示器位置、卡片高度只能由一个公式或一个 token
   推导；禁止手调魔法值、禁止用多次截图“试”到对齐。
5. **移植先逐行对照。** 从 NextE/ErosN 移植组件必须先
   `git diff --no-index` 逐行对照；禁止以“页面不需要”为由删除参考组件
   自身的行为（震动、底部刷新、挂载/卸载生命周期等）。
6. **静态先于构建。** 任何 UI 改动先证明组件树稳定（key 不变、壳层常驻、
   几何不变），再决定是否构建；禁止“安装→滑动→截图→改”的循环作为
   开发与验收方法。
7. **帧级证据要求。** 120Hz 屏幕单帧 8.3ms；`snapshot_display` 循环约
   3-4fps，不能证明单帧不存在。需要帧级证据时使用
   `displaySync.on('frame')`（API 11+）、`UIContext.postFrameCallback`
   （API 12+）、`AVScreenCaptureRecorder` 录屏后逐帧解码，或 DevEco
   Profiler 帧时间线。
8. **同页复查。** 一个可见缺陷修复后，先复查同一父树内的兄弟区域，再离开
   该页面；子组件“修复”不得跳过整页复查。
9. **文本输入必须组件化。** `TextInput` 所在的 `@Builder` 函数禁止把
   可变的输入值作为函数参数传递（如 `PasswordField(text, ...)`）——
   builder 参数变化会重建整个 TextInput 节点，每敲一个字符丢一次焦点、
   收一次输入法。输入值必须由组件自身的 `@Local` 持有
   （`@ComponentV2` 叶子组件或页面自身 `@Local`），builder 只读
   `this.xxx`；复用多个输入框时按稳定布尔/枚举分支，不传文本值。
   新增任何输入框前先 `rg -n -B8 "TextInput\\(" | grep "@Builder"`
   自查；构建通过不等于输入可用，必须真机连续输入两位以上字符验证
   焦点保持。
10. **HDS 菜单图标改动必须先做渲染对照实验。** 两次图标缩小事故的
   共同点：修改 `filterIcon`/`hasActiveFilters` 的取值来源或条件
   结构后，HDS 对该菜单项的渲染路径变化导致图标尺寸改变。因此任何
   触碰图标条件或其状态源的改动，落地前必须在真机截图对照图标
   bounds/像素（原状态 vs 改后，同一图标同一状态），确认尺寸不变
   才能合入；变了就先查渲染路径差异（类型推断、响应式对象依赖、
   SymbolGlyphModifier 分支），而不是试错式来回改。禁止用"冻结
   不许动"代替查因——那会让后续正当需求无法实现。
